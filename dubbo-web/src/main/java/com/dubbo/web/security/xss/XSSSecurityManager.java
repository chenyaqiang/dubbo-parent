 ...
 * @Description:
 * @Author: wb
 * @CreateDate: 2018-04-19 14:09
 * @version:
 **/
public class XSSSecurityManager {

    private static final Log logger = LogFactory.getLog(XSSSecurityManager.class);
    ...
    public static void init(FilterConfig config) {
        if (!isInit) {
            synchronized (XSSSecurityManager.class) {
                ...
                try {
                    if (initConfig(xssPath)) {
                        XSS_PATTERN = Pattern.compile(REGEX, Pattern.CASE_INSENSITIVE);
                        isInit = true;
                    }
    ...
    public static boolean initConfig(String path) throws DocumentException {
        ...
        XSSSecurityConfig.IS_CHECK_HEADER = new Boolean(getEleValue(superElement, XSSSecurityConstant.IS_CHECK_HEADER));
        XSSSecurityConfig.IS_CHECK_PARAMETER = new Boolean(getEleValue(superElement, XSSSecurityConstant.IS_CHECK_PARAMETER));
        XSSSecurityConfig.IS_LOG = new Boolean(getEleValue(superElement, XSSSecurityConstant.IS_LOG));
        XSSSecurityConfig.IS_CHAIN = new Boolean(getEleValue(superElement, XSSSecurityConstant.IS_CHAIN));
        XSSSecurityConfig.REPLACE = new Boolean(getEleValue(superElement, XSSSecurityConstant.REPLACE));
        XSSSecurityConfig.XSS_ERROR_PAGE_PATH = getEleValue(superElement, XSSSecurityConstant.XSS_ERROR_PAGE_PATH);

        Element regexEle = superElement.element(XSSSecurityConstant.REGEX_LIST);
        if (regexEle != null) {
            ...
            logger.error("安全过滤配置文件中没有 " + XSSSecurityConstant.REGEX_LIST + " 属性");
            return false;
        }
    ...
    public static String securityReplace(String text) {
        if (isNullStr(text)) {
            ...
            return text.replaceAll(REGEX, XSSSecurityConstant.REPLACEMENT);
        }
    }
